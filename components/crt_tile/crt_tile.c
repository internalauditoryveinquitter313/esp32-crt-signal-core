                        ...
                        uint8_t *nametable)
{
    ...
    ESP_RETURN_ON_FALSE(visible_w > 0 && visible_h > 0,
                        ESP_ERR_INVALID_ARG, "crt_tile", "zero visible dim");
    ESP_RETURN_ON_FALSE(pitch_w >= visible_w && pitch_h >= visible_h,
                        ESP_ERR_INVALID_ARG, "crt_tile", "pitch smaller than visible");
    ESP_RETURN_ON_FALSE(pattern_table != NULL && pattern_count > 0,
                        ESP_ERR_INVALID_ARG, "crt_tile", "null pattern table");
    ESP_RETURN_ON_FALSE(pattern_count <= 256,
                        ESP_ERR_INVALID_ARG, "crt_tile", "pattern_count > 256");
...
void crt_tile_set_tile(crt_tile_layer_t *t, uint16_t col, uint16_t row,
                       uint8_t tile_idx)
{
    if (t == NULL || t->nametable == NULL) return;
    if (col >= t->pitch_w_tiles || row >= t->pitch_h_tiles) return;
...
{
    if (t == NULL || t->nametable == NULL) return 0;
    if (col >= t->pitch_w_tiles || row >= t->pitch_h_tiles) return 0;
...
static IRAM_ATTR void tile_render_logical_line(const crt_tile_layer_t *t,
                                               uint16_t y,
                                               uint8_t *logical_out)
{
    ...
    const uint16_t w_tiles = t->visible_w_tiles;
    ...
    while (remaining > 0u) {
        ...
        if (idx >= pattern_count) idx = 0;
        const uint8_t *tile_line = &pattern[(size_t)idx * CRT_TILE_BYTES
                                             + (size_t)fine_y * CRT_TILE_PX_W];
...
IRAM_ATTR bool crt_tile_layer_fetch(void *ctx,
                                    uint16_t logical_line,
                                    uint8_t *idx_out,
                                    uint16_t width)
{
    crt_tile_layer_t *t = (crt_tile_layer_t *)ctx;
    if (t == NULL || idx_out == NULL || width == 0u) return false;
    ...
    uint8_t logical_line_buf[256];
    if (logical_w_px > sizeof(logical_line_buf)) {
        memset(idx_out, 0, width);
        return true;
    }
...
IRAM_ATTR void crt_tile_scanline_hook(const crt_scanline_t *scanline,
                                      uint16_t *active_buf,
                                      uint16_t active_width,
                                      void *user_data)
{
    const crt_tile_layer_t *t = (const crt_tile_layer_t *)user_data;
    if (t == NULL || active_buf == NULL || active_width == 0u ||
    ...
    uint32_t step = (((uint32_t)logical_w_px << 16) + (uint32_t)active_width - 1u)
                    / (uint32_t)active_width;
    uint32_t acc = 0;
    const uint16_t even_width = active_width & (uint16_t)~1U;
    uint16_t i = 0;
    for (; i < even_width; i += 2) {
        uint16_t p0 = pal[logical_line_buf[acc >> 16]]; acc += step;
        uint16_t p1 = pal[logical_line_buf[acc >> 16]]; acc += step;
        active_buf[i]     = p1;
        active_buf[i + 1] = p0;
    }
    if (i < active_width) {
